...
function getOrderFallbackMessage(status, order, agentLanguage) {
        ...
        if (agent?.whatsappPhoneId && agent?.whatsappToken) {
          ...
          if (message) {
            message = message
              ...
              .replace(/{tracking}/g, orderWithTracking.trackingNumber  || "")
          } else {
            message = getOrderFallbackMessage(status, orderWithTracking, agent.language || "french")
          }
